...
void GameOverTasks::buttonClickTask(void *statePointer) {
    GameOverState &state = *static_cast<GameOverState*>(statePointer);
    Game &game = Game::get();
...
void GameOverTasks::buttonRenderTask(void *statePointer) {
    GameOverState &state = *static_cast<GameOverState*>(statePointer);
    Game &game = Game::get();
    auto lastWake = xTaskGetTickCount();
